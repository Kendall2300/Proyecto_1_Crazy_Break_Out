...
void Game_Run(struct State *s)
{
    ...
    int /*Deberia de haber alguien aqui no?*/ *key = calloc(ALLEGRO_KEY_MAX, sizeof *key);
...
int Change_State(struct State *s, void *param)
{
    ...
    if (!s->loaded)
    {  void *ptr = realloc(core.loaded_states,sizeof *core.loaded_states * (core.loaded_count + 1));
